...
class ShippingAddressValidator implements RelationValidatorInterface
{
    /**
     * @var string|null
     */
    ...
    {
        if (isset($entityRow['shipping_address_id']) && empty($subEntityRows)) {
            $this->message = __(
                'No shipping address provided for order %1',
                $entityRow['entity_id']
            )->render();
        ...
        foreach ($subEntityRows as $subEntityRow) {
            if (!$this->checkAddressId($entityRow, $subEntityRow)) {
                $this->message = __(
                    'Wrong shipping address specified for order %1',
                    $entityRow['entity_id'] ?? ''
                )->render();
    ...
    private function checkAddressId(array $entityRow, array $subEntityRow): bool
    {
        if (!isset($subEntityRow['entity_id'])
            || !isset($entityRow['shipping_address_id'])
            || $entityRow['shipping_address_id'] != $subEntityRow['entity_id']
        ) {
            return false;
        }
