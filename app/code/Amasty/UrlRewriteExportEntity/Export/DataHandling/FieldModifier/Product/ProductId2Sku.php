...
namespace Amasty\UrlRewriteExportEntity\Export\DataHandling\FieldModifier\Product;
...
class ProductId2Sku extends AbstractModifier implements FieldModifierInterface
{
    ...
    {
        if (!empty($value)) {
            if ($sku = $this->getSkuByEntityId((int)$value)) {
                return $sku;
            }
        }
    ...
    {
        return __('Product Id To SKU')->render();
    }
     ...
     * @throws \Exception
     */
    private function getSkuByEntityId(int $entityId): string
    {
