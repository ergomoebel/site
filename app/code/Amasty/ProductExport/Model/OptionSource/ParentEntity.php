...
class ParentEntity implements OptionSourceInterface
{
    ...
    {
        $productEntityConfig = $this->entityConfigProvider->get('catalog_product_entity');
        $options = [
            ['value' => '', 'label' => __('-- Please Select --')],
            ['value' => $productEntityConfig->getEntityCode(), 'label' => __($productEntityConfig->getName())]
        ];

        return array_merge(
            $options,
            $this->getNestedOptions($this->relationConfigProvider->get($productEntityConfig->getEntityCode()))
        );
    }
